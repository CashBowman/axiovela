...
import React, {lazy, Suspense, useCallback, useMemo, useEffect, useId, useRef, useState} from 'react';
...
function useMarkdownAssets(root, artifacts, localOnly = false) {
  const revisions = JSON.stringify((artifacts || []).map(item => [item.path, item.modifiedAt || '']));
  const mapping = useMemo(() => new Map(JSON.parse(revisions)), [revisions]);
  return useCallback(src => {
    const normalized = src.replace(/^\.\.\//, '').replace(/^\.\//, '');
    if (!/^(artifacts\/figures|exports)\//.test(normalized)) return localOnly ? '' : src;
    const url = markdownAssetUrl(src, root), revision = mapping.get(normalized);
    return revision ? url + '&v=' + encodeURIComponent(revision) : url;
  }, [root, mapping, localOnly]);
}
...
function ChatbotPane({artifacts, annotationCount=0,request,onApplied,beforeApply,title = 'Experiment Chatbot', messages, input, setInput, send, attach, dropAttachment, attachmentName, clearAttachment, busy, attachmentBusy, assistantProgress, assistantEvents = [], lastActivityAt, statusError, agenticActivity, permissionMode, setPermissionMode, cancelAssistant, modelControl, conversationControl, queueControl, engineName, renderDocument, supportedModes, projectRoot, ready, agenticMode = false, setAgenticMode, agentic, agenticStarting}) {
  ...
  const imageUrl = useMarkdownAssets(projectRoot, artifacts, true);
  ...
  return <Pane title={title} tag={{label: engineName, tone: 'purple'}} action={agenticAction}><div className="ai cleanChat">
    ...
    <div ref={scroll.viewport} onScroll={scroll.onScroll} className="chat" aria-label="Conversation"><div ref={scroll.content} className="chatMessages">
      {messages.map(([who, text, id, role, record], i) => <div key={id || i} className={'message ' + (who === 'bot' ? 'ai' : who)}><div className="messageBody">
        {who === 'bot' ? <Suspense fallback={<span>{!id ? welcome : text}</span>}><ChatContent source={chatMarkdown(!id ? welcome : text, projectRoot)} assetUrl={imageUrl} renderDocument={renderDocument}/></Suspense> : <PromptText text={text}/>}
...
function App() {
  ...
  const parallel = useParallelAssistant({request: unscopedRequest,
    onStarted: (record, item) => {
      ...
      setConversations(current => {
        const previous = current.find(c => c.id === record.conversationId);
        const title = previous && ['manual', 'preserved'].includes(previous.titleSource) ? previous.title : record.conversationTitle;
        return [{id: record.conversationId, role: record.role, createdAt: record.startedAt, ...previous, title, updatedAt: record.startedAt}, ...current.filter(c => c.id !== record.conversationId)];
      });
  ...
  const experimentRunning = experimentRecords.find(record => record.status === 'running');
  const lastExperiment = experimentRecords.reduce((latest, record) => !latest || record.startedAt > latest.startedAt ? record : latest, null);
  const agenticMode = experimentRunning?.agenticMode ?? agenticChoices[agenticKey] ?? lastExperiment?.agenticMode ?? false;
  const setAgenticModeState = enabled => setAgenticChoices(current => {
    const next = {...current, [agenticKey]: enabled};
    try { localStorage.setItem('axiovela-agentic-choices', JSON.stringify(next)); } catch {}
    return next;
  });
  ...
  useEffect(()=>{const jump=e=>{const editor=writeupEditorRef.current;if(!editor)return;const lines=editor.value.split('\n'),line=Math.max(1,Math.min(lines.length,Number(e.detail)||1)),at=lines.slice(0,line-1).reduce((total,value)=>total+value.length+1,0),lineHeight=parseFloat(getComputedStyle(editor).lineHeight)||20;editor.focus();editor.setSelectionRange(at,at+lines[line-1].length);editor.scrollTop=Math.max(0,(line-1)*lineHeight-editor.clientHeight/3);};window.addEventListener('axiovela-source-line',jump);return()=>window.removeEventListener('axiovela-source-line',jump);},[]);
  ...
  const chooseConversations = (next, root = projectInfo?.root) => {
    setActiveConversations(old => JSON.stringify(old) === JSON.stringify(next) ? old : next);
  ...
  const applyAssistantHistory = (history, root) => {
    ...
    setConversations(old => JSON.stringify(old) === JSON.stringify(available) ? old : available);
    chooseConversations(Object.fromEntries(['experiment', 'writing'].map(role => [role,
      ...
      history.jobs?.find(job => job.role === role && job.status === 'running')?.conversationId || available.find(item => item.role === role && !item.archived)?.id || '',
    ])), root);
    parallel.adopt(history);
    setMessages(old => { const next = assistantHistoryMessages(history.jobs); return JSON.stringify(old) === JSON.stringify(next) ? old : next; });
  };
...
\\end{figure}` : `![${caption}](${artifact.path})`;
  ...
  const compileLatex = async () => {
    ...
    try { await saveSource(); const source = draftValue.current, annotationBibliography = bibliography.source; const result = await apiRequest('/api/writeups/compile', {method: 'POST', body: JSON.stringify({draftText: source, expectedSource: source})}); setCompileResult({...result,annotationSource:source,annotationBibliography}); await refreshProject(); }
    catch (error) {
  ...
  const {recentConversations, titleCounts} = useMemo(() => {
    ...
    const recentConversations = items.filter(item => !item.archived).sort((a, b) => Number(Boolean(b.pinned)) - Number(Boolean(a.pinned)) || String(b.updatedAt || b.createdAt).localeCompare(String(a.updatedAt || a.createdAt))).slice(0, 50);
    const selected = items.find(item => item.id === conversationId);
    if (selected && !recentConversations.some(item => item.id === conversationId)) recentConversations.unshift(selected);
    return {recentConversations, titleCounts};
  }, [conversations, role, conversationId]);
  const conversationControl = <><div className="conversationToolbar"><label>Conversation<select aria-label="Conversation" value={conversationId} disabled={conversationBusy || !historyReady} onChange={event => { setConversationError(''); chooseConversations({...activeConversations, [role]: event.target.value}); }}>{!conversationId && <option value="">New conversation</option>}{recentConversations.map(item => <option key={item.id} value={item.id}>{item.pinned ? '★ ' : ''}{item.title}{item.archived ? ' (archived)' : ''}{titleCounts.get(item.title) > 1 ? ` · ${item.id.slice(-8)}` : ''}</option>)}</select></label><button onClick={newConversation} disabled={conversationBusy || !historyReady || !projectInfo?.root} title="Start a new conversation"><Plus size={14}/> New chat</button><ConversationHistory request={apiRequest} root={projectInfo?.root} disabled={conversationBusy || projectBusy || attachmentBusy || sourceBusy || !historyReady} onUpdated={(conversation, root) => { if (root === projectRootRef.current) setConversations(current => current.map(item => item.id === conversation.id ? conversation : item)); }} onOpen={async item => {
    if (item.projectRoot !== projectRootRef.current) {
      const project = await openProject(item.projectRoot, false, false);
      if (!project || project.root !== item.projectRoot) throw Error('Could not open the saved project. Its files were not changed.');
    }
    setActiveConversations(current => {
      const next = {...current, [item.role]: item.id};
      try { localStorage.setItem(`ml-workbench-conversations:${item.projectRoot}`, JSON.stringify(next)); } catch {}
      return next;
    });
    setTab(item.role === 'writing' ? 'Write-up' : 'Results');
