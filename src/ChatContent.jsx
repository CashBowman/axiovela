...
export default function ChatContent({source, assetUrl, renderDocument}) {
  ...
  return <div className="chatContent"><MarkdownPreview copyCode source={readable} assetUrl={assetUrl} onLink={href=>{const p=context?.sources?.find(p=>href===p.sourceUrl||href==="#library-source-"+p.id);if(p){context.openSource(p.id);return true;}return false;}}/>{formatted.followups.map((f,i)=><button className="chatDocumentButton" key={i} onClick={()=>context?.draftFollowup(f.prompt)}>{f.label}</button>)}{documents.map((document, i) => <span key={i}><button className="chatDocumentButton" onClick={() => downloadSource(document.format, document.source)}>Export source</button><button className="chatDocumentButton" disabled={busy} onClick={() => render(document, true)}>Export PDF</button><button className="chatDocumentButton" disabled={busy} onClick={() => render(document)}>{busy ? 'Rendering…' : `Render ${document.format === 'latex' ? 'LaTeX PDF' : 'Markdown document'}${documents.length > 1 ? ` ${i + 1}` : ''}`}</button></span>)}{error && <p role="alert" className="renderError">{error}</p>}{result?.url && <div className="chatDocumentPreview"><a href={result.url} target="_blank" rel="noopener noreferrer">Open rendered document</a>{result.format === 'latex' && <SavedDocumentReader title="Chat LaTeX document" url={result.url}/>}</div>}</div>;
}
