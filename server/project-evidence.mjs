...
- For figure edits, inspect artifacts/figures/metadata.json, the target image, and its existing plotting source; reuse saved results and revise the same export path by default. New experiments and explicitly requested variants use separate paths. For data, read datasets/catalog.json, then retrieve only relevant schemas and bounded samples.
