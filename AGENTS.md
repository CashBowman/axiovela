...
- Parallel conversations: `src/useParallelAssistant.js` owns task polling and per-conversation follow-up queues. `server/index.mjs` captures request roots with AsyncLocalStorage; preserve `x-axiovela-project` through the desktop bridge. Keep job reads/cancellation scoped and admit only one turn per conversation. Run `npm run parallel:smoke` for overlapping roles/projects, hidden queues, stalled status/upload isolation and reload recovery. Keep activity freshness separate from timeline deduplication and successful status polls. `monitorAgenticActivity` schedules after completion to avoid Herdr poll backlogs; `scripts/pi-activity-smoke.mjs` covers Pi Agentic wait/reconnect/cancel behavior. Publish cancellation only after provider cleanup releases the conversation. Queued prompts and edits persist locally and restore paused; failure/Stop retains the queue. Keep edit locks and FIFO ordering. `agentic-queue-smoke.mjs` tests error retention, edits, reload and dispatch. Pi `agent_end` may precede retries/compaction: prefer `agent_settled`, preserving the legacy idle-probe fallback. This is shared-project concurrency, not worktree isolation.

- Library and passage annotations: `docs/annotations.md`, `server/library.mjs`, `server/annotations.mjs`. Run `npm run annotations:test` and `npm run annotations:smoke` (or set `AXIOVELA_ANNOTATION_TEST_BINARY` for packaged acceptance). Preserve native selection, explicit Add/Send, project/conversation scoping, immutable PDF revisions, and read-only manuscript proposals with revision-checked Apply. `shared/` is a packaged runtime directory. Library documents remain mounted while switching workspaces. `npm run library:navigation:smoke` checks filtered group order, roving focus and independent read controls; list rendering and keyboard order must share the same grouped items. `shared/research-outline.mjs` defines science/engineering result kinds; `research/connections.json` drives Library items and explanations. Keep stable IDs, main-result markers and measured provenance separate from assistant interpretations.

- Project switcher and cross-project map: `docs/project-navigator.md`, `server/project-catalog.mjs`, `src/ProjectNavigator.jsx`. `npm run projects:test` and `npm run projects:smoke` use isolated fixtures. Keep device-local history separate from open tabs, bound index reads, preserve stable IDs on relocation, and never recreate missing projects when reopening history. Derived shared-source links are associations, not scientific conclusions.

- Interaction rendering: `npm run interaction:smoke` checks memo invalidation and live callbacks; `PERF_LABEL=local PERF_ASSERT=1 npm run interaction:perf` retains CPU profiles and whole typing-sequence timings in an isolated long-document fixture. Preserve capture-time viewport rectangles and ignore overlay-only mutations. Image mapping dependencies include artifact byte revisions (`modifiedAt`); callback identity alone must not invalidate Markdown/math.

- Conversation history: `docs/conversation-history.md`, `server/assistant-conversations.mjs`, `server/conversation-search.mjs`. Run `npm run history:test` and `npm run history:smoke`. Metadata-only upgrades back up original JSON under each project's `assistant/conversation-backups/v1/`; preserve job/session files and logical IDs. Cross-project search is bounded/read-only and grants no project access. Manual titles survive first send; provider naming is nonfatal and synchronized on the next turn. Codex sidebar cleanup lives in `server/assistant-runtime.mjs`; `npm run codex:sidebar:test` covers root-only completed-turn cleanup, bounded idle/loaded checks and archive/resume recovery. Native archive state must never change Axiovela History state or queue ownership.
