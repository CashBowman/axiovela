...
try {
 ...
 await page.keyboard.press('Enter');await page.getByRole('dialog',{name:'Figure 0'}).waitFor();await page.keyboard.press('Shift+Equal');assert.equal(await page.locator('.viewerTools output').innerText(),'125%');await page.keyboard.press('Shift+ArrowRight');assert.equal(await page.locator('#figure-viewer-title').innerText(),'Figure 0');await page.keyboard.press('Control+ArrowRight');assert.equal(await page.locator('#figure-viewer-title').innerText(),'Figure 0');await page.keyboard.press('ArrowRight');assert.equal(await page.locator('#figure-viewer-title').innerText(),'Figure 1');await page.keyboard.press('Escape');await page.locator('.figureViewer').waitFor({state:'hidden'});
